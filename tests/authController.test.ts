...
// Test functions yang diekstrak dari AuthController
...
describe('Auth Logic', () => {
  describe('isAccessExpired', () => {
    it('null/undefined tidak expired', () => {
      expect(isAccessExpired(null)).toBe(false)
      expect(isAccessExpired(undefined)).toBe(false)
    })

    it('tanggal lampau dianggap expired', () => {
      expect(isAccessExpired('2020-01-01T00:00:00Z')).toBe(true)
    })
    ...
    it('tanggal invalid tidak expired', () => {
      expect(isAccessExpired('invalid-date')).toBe(false)
    })
  })

  describe('getAccessDaysRemaining', () => {
    it('null input returns null', () => {
      expect(getAccessDaysRemaining(null)).toBeNull()
    })

    it('invalid date returns null', () => {
      expect(getAccessDaysRemaining('not-a-date')).toBeNull()
    })

    it('expired date returns 0', () => {
      expect(getAccessDaysRemaining('2020-01-01')).toBe(0)
    })
