...
export function RequireMinRole({ children, minRole, fallback = '/app' }: GuardProps & { minRole: AppRole }) {
  const { user } = useAuth()
  if (!user) return <Navigate to="/login" replace />
...
export function RequireRoles({ children, allowedRoles, fallback = '/app' }: GuardProps & { allowedRoles: AppRole[] }) {
  const { user } = useAuth()
  if (!user) return <Navigate to="/login" replace />
...
export function RequireOperationalAdmin({ children, fallback = '/app' }: GuardProps) {
  const { user } = useAuth()
  if (!user) return <Navigate to="/login" replace />
  if (user.hak_akses === 'demo') return <Navigate to={fallback} replace />
...
export function RequireDeveloperPanel({ children, fallback = '/app' }: GuardProps) {
  const { user } = useAuth()
  if (!user) return <Navigate to="/login" replace />
